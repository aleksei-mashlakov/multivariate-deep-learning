...
def optimize(optimizer_params):
    ...
    logger = TestTubeLogger("tb_logs", name="my_dsanet_ele")
...
if __name__ == '__main__':
    ...
    ITERATION = 0
    out_file = os.path.join('./save', 'train_results_electricity.csv')
    results_pickle_file = os.path.join('./save', 'train_results_electricity_pickle.pkl')
    nb_evals = 1
    max_evals = 1
    ...
    while max_evals <= 100:
        ...
        search_params = {'dropout': hp.quniform('dropout', 0.1, 0.5, 0.1),
                         'batch_size': 2 ** hp.quniform('batch_size', 6, 8, 1),
