...
if __name__ == '__main__':
    ...
    ITERATION = 0
    out_file = os.path.join('./save', 'train_results_power_v3.csv')
    results_pickle_file = os.path.join('./save', 'train_results_power_pickle_v3.pkl')
    nb_evals = 5
    max_evals = 1
    ...
    while max_evals <= 150:
        ...
        search_params = {'dropout': hp.quniform('dropout', 0.1, 0.5, 0.1),
                         ...
                         'n_kernels': hp.choice('n_kernels', [32, 50]),
                         'window': hp.choice('window', [128, 168])}
        best = fmin(optimize,
                    search_params,
                    algo=tpe.suggest,
                    max_evals=max_evals,
